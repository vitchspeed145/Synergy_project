import tkinter as tk # импортируем ткинтер для разработки интерфейса
...
class DB:
    def __init__(self):
        self.conn = sqlite3.connect('db.db') # создаем коннект с базой данных (соединение) 
        self.c = self.conn.cursor() # создание объекта класса cursor для взаимодействия с базой данных

        # ниже отправка запроса в базу данных через execute
        self.c.execute(
    ...
    def insert_data(self, name, tel, email, salary):
        ...
        self.conn.commit() # сохраняем изменения базы данных

if __name__ == '__main__':
    root = tk.Tk()
    db = DB() # объявляем экземпляр класса DB
    app = Main(root)
    app.pack()
    root.title('Список сотрудников компании') # отмечаем заголовок окна
    root.geometry('800x600')     # задаем размер окна
    # ограничиваем изменения размеров окна
